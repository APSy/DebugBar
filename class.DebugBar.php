...
class DebugBar {
    ...
    private static function printJS () {
        echo '<script type="text/javascript" src="'.self::$path.'/DebugBar.js"></script>';
        echo '<script type="text/javascript">init(' . self::$totalTime . ','. self::$mem .', {get:'.json_encode($_GET).',length:'.count($_GET).'}, {post:'.json_encode($_POST).', length:'.count($_POST).'} , {session:'.json_encode($_SESSION).', length:'.count($_SESSION).' }, {watched:'.json_encode(self::$watched).', length:'.count(self::$watched).'}, '.json_encode(self::$options).', '.self::$path.' )</script>';
    }
